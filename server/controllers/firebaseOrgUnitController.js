...
const syncFromCommunes = asyncHandler(async (req, res) => {
    const result = await firebaseOrgUnitService.syncFromCommunes(req.body || {});
    res.status(200).json({
        success: true,
        data: result,
        message: `Đồng bộ xong: tạo mới ${result.createdCount}, bỏ qua ${result.skippedCount}`,
    });
});
